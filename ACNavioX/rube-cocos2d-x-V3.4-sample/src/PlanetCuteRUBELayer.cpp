...
PlanetCuteRUBELayer::PlanetCuteRUBELayer()
{
    m_playerBody = NULL;
    m_numFootContacts = 0;
...
Point PlanetCuteRUBELayer::initialWorldOffset()
{
...
void PlanetCuteRUBELayer::afterLoadProcessing(b2dJson* json)
{
    ...
    SimpleAudioEngine::getInstance()->preloadEffect("sounds/jump.wav");
    SimpleAudioEngine::getInstance()->preloadEffect("sounds/pickupgem.wav");
    SimpleAudioEngine::getInstance()->preloadEffect("sounds/pickupstar.wav");
...
{
    SimpleAudioEngine::getInstance()->unloadEffect("sounds/jump.wav");
    SimpleAudioEngine::getInstance()->unloadEffect("sounds/pickupgem.wav");
    SimpleAudioEngine::getInstance()->unloadEffect("sounds/pickupstar.wav");
...
void PlanetCuteRUBELayer::update(float dt)
{
    ...
    for (set<PlanetCuteFixtureUserData*>::iterator it = m_pickupsToProcess.begin(); it != m_pickupsToProcess.end(); ++it) {
        ...
        if ( fud->pickupType == PT_GEM )
            SimpleAudioEngine::getInstance()->playEffect("sounds/pickupgem.wav", false);
        else if ( fud->pickupType == PT_STAR ) {
            SimpleAudioEngine::getInstance()->playEffect("sounds/pickupstar.wav", false);
    ...
    if ( m_leftTouch && m_rightTouch && m_numFootContacts > 0 && m_jumpTimeout <= 0 ) {
        ...
        SimpleAudioEngine::getInstance()->playEffect("sounds/jump.wav");
        if ( m_instructionsSprite2 ) {
    ...
    Size s = Director::getInstance()->getWinSize();
    Point sceneOffset = Vec2(s.width/2 - cameraCenterInPixels.x, s.height/2 - cameraCenterInPixels.y);
    setPosition(sceneOffset);
}


// record when the user touches the left/right sides of the screen
void PlanetCuteRUBELayer::onTouchesBegan(const std::vector<cocos2d::Touch*>& touches, cocos2d::Event *unused_event)
{
    Size s = Director::getInstance()->getWinSize();
...
void PlanetCuteRUBELayer::onTouchesMoved(const std::vector<cocos2d::Touch*>& touches, cocos2d::Event *unused_event)
{
...
// screen sides that we recorded in TouchesBegan. If so, set the current
// touch for that screen side to nil
void PlanetCuteRUBELayer::onTouchesEnded(const std::vector<cocos2d::Touch*>& touches, cocos2d::Event *unused_event)
{
    for (int i = 0; i < touches.size(); i++) {
        Touch* touch = touches[i];
