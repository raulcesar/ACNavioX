...
static b2Vec2 ComputeCentroid(std::vector<b2Vec2> vs, float& area)
{
    ...
    for (int32 i = 0; i < count; ++i)
    {
        ...
        float32 D = b2Cross(p2, p3);
        CCLOG("b2Cros of (%f, %f) and (%f, %f) is %f", p2.x, p2.y, p3.x, p3.y, D);
...
//check if point p is inside clip egde (defined by points cp1 and cp2)
...
bool findIntersectionOfFixtures(b2Fixture* fA, b2Fixture* fB, std::vector<b2Vec2>& outputVertices)
{
    ...
    b2PolygonShape* polygonSubject = (b2PolygonShape*)fA->GetShape();
    b2PolygonShape* polygonClip = (b2PolygonShape*)fB->GetShape();

    //fill 'subject polygon' from fixtureA polygon
    for (int32 i = 0; i < polygonSubject->GetVertexCount(); i++){
        outputVertices.push_back( fA->GetBody()->GetWorldPoint( polygonSubject->GetVertex(i) ) );
    }

    //fill 'clip polygon' from fixtureB polygon
    std::vector<b2Vec2> clipPolygon;
    for (int32 i = 0; i < polygonClip->GetVertexCount(); i++){
        clipPolygon.push_back( fB->GetBody()->GetWorldPoint( polygonClip->GetVertex(i) ) );
    }
