...
public:
    ...
    {
        switch (key) {
        case 'a' : m_boatDrive = false; break;
        default: Test::Keyboard(key);
        }
    }
    ...
    {
        m_debugDraw.DrawString(5, m_textLine, "Press a to drive the boat");
        m_textLine += 15;
        ...
        if ( m_boatBody ) {
            ...
            if ( m_boatDrive && m_boatBody->GetPosition().y < 10.1 ) {
                ...
                m_boatBody->ApplyForce( m_boatBody->GetWorldVector( b2Vec2(400,-25) ), m_boatBody->GetPosition() );
